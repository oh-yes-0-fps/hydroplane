//! `y = a·x + y` elementwise: one FMA per element, memory-bandwidth-bound.
...
// Memory-bound (~0.16 flop/byte), so this uses the streaming combinator (a scalar closure over an
// auto-vectorized loop) rather than the SIMD `map` family. `#[hint_cnt]` records the expected
